...
let posicComienzo = 0;   // el punto DESDE donde se parte de la imagen => iría de 0 a 300  ( 150 sería la 1/2 del ancho que dimos de 300 -> iría de 150 a 450)
const slideWidth = document.querySelector('img').offsetWidth;    //  sliderWidth --> es el ancho de CADA slide
...
const slidesCount= document.querySelectorAll('img').length;
const totalWidth = slidesCount * slideWidth;
const primeraImg = 0;
const ultimaImg = -(totalWidth - slideWidth);

next.addEventListener('click', function(){
    if(posicComienzo > ultimaImg) {
        posicComienzo -= slideWidth;
    } else {
        posicComienzo = primeraImg;
    }
    slider.style.left = posicComienzo + 'px';
})

prev.addEventListener('click', function(){
    if(posicComienzo < primeraImg) {
        posicComienzo += slideWidth;
    }else{
        posicComienzo = ultimaImg; 
    }
    slider.style.left = posicComienzo + 'px';
})
